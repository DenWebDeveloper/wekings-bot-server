...
module.exports.gifts = Joi.object().keys({
    ...
    gifts: Joi.array().items(Joi.object().keys({
        userId: Joi.number().integer().min(0).max(100000).required(),
        giftId: Joi.number().integer().min(0).max(100000).required(),
    }))
})
...
module.exports.curses = Joi.object().keys({
    ...
    curses: Joi.array().items(Joi.object().keys({
        userId: Joi.number().integer().min(0).max(100000).required(),
        curseId: Joi.number().integer().min(0).max(100000).required(),
    }))
})
